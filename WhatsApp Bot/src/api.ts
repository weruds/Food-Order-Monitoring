...
// Verify a phone number is registered on WhatsApp before sending.
// Returns the canonical JID string, or null if not found.
...
async function sendDM(
  ...
  if (!jid) {
    const err: any = new Error(`${phone} is not registered on WhatsApp`);
    err.notFound = true;
    throw err;
  }

  await sock.sendMessage(jid, content);
  console.log(`[API] ${label} sent → ${jid}`);
  return { jid };
}
...
-admin`;
    ...
    try {
      ...
      await sendDM(cleanPhone, { text: message }, `notify-assignee → ${name}`);
      res.json({ ok: true });
    } catch (err: any) {
      console.error('[API] Failed to send assignee notification:', err);
      if (err.notFound) {
        res.status(404).json({ error: `${phone} is not registered on WhatsApp` });
      } else {
